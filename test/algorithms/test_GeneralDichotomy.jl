...
# Toy instance from Przybylski et al. (2019).
function test_lap()
    ...
    MOI.add_constraint.(model, x_, MOI.ZeroOne())
    x = reshape(x_, n, n)
    for i in 1:n
        MOI.add_constraint(model, ones(n)' * x[i, :], MOI.EqualTo(1.0))
        MOI.add_constraint(model, ones(n)' * x[:, i], MOI.EqualTo(1.0))
    end
...
# Testing weight set cleaning on a random lap instance.
function test_lap_2()
...
# Test instance from Bensolve (http://www.bensolve.org/).
function test_vlp()
    ...
    MOI.add_constraints(model, B * x, MOI.GreaterThan.(a))
    MOI.set(model, MOI.ObjectiveSense(), MOI.MIN_SENSE)
    ...
    y_sol = round.(Int, 10 .* MOI.get.(model, MOI.ObjectiveValue.(1:N)))
